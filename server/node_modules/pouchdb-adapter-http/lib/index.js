...
function hasUrlPrefix(opts) {
  ...
  let protocol = pouchdbUtils.parseUri(opts.prefix).protocol;
...
function getHost(name, opts) {
  ...
  let uri = pouchdbUtils.parseUri(name);
...
function HttpPouch(opts, callback) {
  ...
  const ourFetch = async function (url, options) {
    ...
    if (opts.auth || host.auth) {
      ...
      let token = pouchdbBinaryUtils.btoa(unescape(encodeURIComponent(str)));
    ...
    let fetchFun = opts.fetch || pouchdbFetch.fetch;
  ...
  async function fetchJSON(url, options) {
    ...
    if (!result.ok) {
      ...
      let err = pouchdbErrors.generateErrorFromResponse(result.data);
      throw err;
    }
  ...
  api.bulkGet = pouchdbUtils.adapterFun('bulkGet', function (opts, callback) {
    ...
    function doBulkGetShim() {
      ...
      for (let i = 0; i < numBatches; i++) {
        let subOpts = pouchdbUtils.pick(opts, ['revs', 'attachments', 'binary', 'latest']);
  ...
  api._changes = function (opts) {
    ...
    let controller = new pouchdbFetch.AbortController();
    let lastFetchedSeq;
    ...
    const fetched = function (err, res) {
      ...
      if (res && res.results) {
        ...
        res.results = res.results.filter(function (c) {
          leftToFetch--;
          let ret = pouchdbUtils.filterChange(opts)(c);
          if (ret) {
            if (opts.include_docs && opts.attachments && opts.binary) {
